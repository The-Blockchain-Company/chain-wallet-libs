...
CDVPluginResult*
quibitous_error_to_plugin_result(ErrorPtr error)
{
    char* error_desc_raw = tbco_quibitous_wallet_error_to_string(error);
    ...
    tbco_quibitous_wallet_delete_string(error_desc_raw);
    tbco_quibitous_wallet_delete_error(error);
...
- (void)WALLET_IMPORT_KEYS:(CDVInvokedUrlCommand*)command
{
    ...
    [self.commandDelegate runInBackground:^{
        ...
        ErrorPtr result = tbco_quibitous_wallet_import_keys(account_key.bytes,
            utxo_keys.bytes,
            utxo_keys.length / 64,
            &wallet_ptr);

        if (result != nil) {
            pluginResult = quibitous_error_to_plugin_result(result);
        } else {
...
- (void)SYMMETRIC_CIPHER_DECRYPT:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr result = tbco_quibitous_symmetric_cipher_decrypt(password.bytes,
        password.length,
        ciphertext.bytes,
        ciphertext.length,
        &plaintext,
...
- (void)WALLET_SPENDING_COUNTER:(CDVInvokedUrlCommand*)command
{
    ...
    uint32_t value;
    ErrorPtr result = tbco_quibitous_wallet_spending_counter(wallet_ptr, &value);
...
- (void)WALLET_TOTAL_FUNDS:(CDVInvokedUrlCommand*)command
{
    ...
    WalletPtr wallet_ptr = (WalletPtr)[wallet_ptr_raw longLongValue];
    uint64_t value;
    ErrorPtr result = tbco_quibitous_wallet_total_value(wallet_ptr, &value);
...
- (void)WALLET_ID:(CDVInvokedUrlCommand*)command
{
    ...
    WalletPtr wallet_ptr = (WalletPtr)[wallet_ptr_raw longLongValue];
    uint8_t id_ptr[32];
    ErrorPtr result = tbco_quibitous_wallet_id(wallet_ptr, id_ptr);
...
- (void)WALLET_SET_STATE:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr result = tbco_quibitous_wallet_set_state(wallet_ptr, value, counter);
...
- (void)WALLET_VOTE:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr result = tbco_quibitous_wallet_vote_cast(wallet_ptr,
        settings_ptr,
        proposal_ptr,
        choice,
        date,
    ...
    if (result != nil) {
        ...
        tbco_quibitous_wallet_delete_buffer(transaction_out, len_out);
    }
...
- (void)PROPOSAL_NEW_PUBLIC:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr result = tbco_quibitous_vote_proposal_new_public(vote_plan_id.bytes,
        index,
        num_choices,
        &proposal_out_ptr);
...
- (void)PROPOSAL_NEW_PRIVATE:(CDVInvokedUrlCommand*)command
{
    ...
    ProposalPtr proposal_out_ptr = nil;
    ErrorPtr result = tbco_quibitous_vote_proposal_new_private(vote_plan_id.bytes,
        index,
        num_choices,
...
- (void)SETTINGS_NEW:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr result = tbco_quibitous_wallet_settings_new(settings_init, &settings_out_ptr);
...
- (void)SETTINGS_GET:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr discrimination_result =
        tbco_quibitous_wallet_settings_discrimination(settings_ptr, &discrimination);
    ErrorPtr block0_hash_result =
        tbco_quibitous_wallet_settings_block0_hash(settings_ptr, &block0_hash);
    ErrorPtr linear_fees_result = tbco_quibitous_wallet_settings_fees(settings_ptr, &linear_fees);

    if (discrimination_result != nil) {
        pluginResult = quibitous_error_to_plugin_result(discrimination_result);
    } else if (block0_hash_result != nil) {
        pluginResult = quibitous_error_to_plugin_result(block0_hash_result);
    } else if (linear_fees_result != nil) {
        pluginResult = quibitous_error_to_plugin_result(linear_fees_result);
    } else {
        NSDictionary* result = @{
...
- (void)FRAGMENT_ID:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr result_fragment_from_raw =
        tbco_quibitous_fragment_from_raw(fragment_raw.bytes, fragment_raw.length, &fragment_ptr);

    if (result_fragment_from_raw != nil) {
        pluginResult = quibitous_error_to_plugin_result(result_fragment_from_raw);
    } else {
        uint8_t fragment_id[32];
        ErrorPtr result_fragment_id = tbco_quibitous_fragment_id(fragment_ptr, fragment_id);

        if (result_fragment_id != nil) {
            pluginResult = quibitous_error_to_plugin_result(result_fragment_id);
        } else {
        ...
        tbco_quibitous_delete_fragment(fragment_ptr);
    }
...
- (void)BLOCK_DATE_FROM_SYSTEM_TIME:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr result_c_call =
        tbco_quibitous_block_date_from_system_time(settings_ptr, date, &block_date_out);

    if (result_c_call != nil) {
        pluginResult = quibitous_error_to_plugin_result(result_c_call);
    } else {
        NSDictionary* result = @{
...
- (void)MAX_EXPIRATION_DATE:(CDVInvokedUrlCommand*)command
{
    ...
    ErrorPtr result_c_call =
        tbco_quibitous_max_expiration_date(settings_ptr, date, &block_date_out);
...
- (void)WALLET_DELETE:(CDVInvokedUrlCommand*)command
{
    NSString* wallet_ptr_raw = [command.arguments objectAtIndex:0];
    WalletPtr wallet_ptr = (WalletPtr)[wallet_ptr_raw longLongValue];
    tbco_quibitous_wallet_delete_wallet(wallet_ptr);
...
- (void)SETTINGS_DELETE:(CDVInvokedUrlCommand*)command
{
    NSString* settings_ptr_raw = [command.arguments objectAtIndex:0];
    SettingsPtr settings_ptr = (SettingsPtr)[settings_ptr_raw longLongValue];
    tbco_quibitous_wallet_delete_settings(settings_ptr);
    CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
...
- (void)PROPOSAL_DELETE:(CDVInvokedUrlCommand*)command
{
    ...
    ProposalPtr proposal_ptr = (ProposalPtr)[proposal_ptr_raw longLongValue];
    tbco_quibitous_wallet_delete_proposal(proposal_ptr);
    CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
